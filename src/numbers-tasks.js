...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const valueOfFirstVector = Math.sqrt(x1 ** 2 + y1 ** 2);
  const valueOfSecondVector = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(twoVectors / (valueOfFirstVector * valueOfSecondVector));
}
